...
"""
v8DistDetectionLoss — Loss function pre Dist-YOLO

Pridá 4. loss komponent: L1 loss medzi predikovanou a skutočnou vzdialenosťou
pre každý foreground anchor.
"""


class v8DistDetectionLoss(v8DetectionLoss):

    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):
        """Initialize Dist-YOLO loss."""
        super().__init__(model, tal_topk, tal_topk2)

        # L1 loss pre distance regression
        self.dist_loss = nn.L1Loss(reduction='none')

        # Distance loss weight (default 1.0, možno override cez hyp)
        # hyp.dist môže byť zaregistrovaný v default.yaml
        self.dist_gain = getattr(model.args, 'dist', 1.0)

    def loss(self, preds, batch):
        """Calculate detection + distance loss.

        Args:
            preds (dict): Model predictions with 'boxes', 'scores', 'distances', 'feats'.
            batch (dict): Ground truth with 'batch_idx', 'cls', 'bboxes', 'distances'.

        Returns:
            tuple: (total_loss * batch_size, loss_detach_per_component)
        """
        batch_size = preds["boxes"].shape[0]

        # Volá bežný assigner — dostaneme fg_mask, target_gt_idx + box/cls/dfl loss
        (fg_mask, target_gt_idx, target_bboxes, anchor_points, stride_tensor), \
            det_loss, det_loss_detach = self.get_assigned_targets_and_loss(preds, batch)

        # Rozšíreny loss tensor — 4 komponenty (box, cls, dfl, dist)
        loss = torch.zeros(4, device=self.device)
        loss[:3] = det_loss  # box, cls, dfl

        # === DISTANCE LOSS ===
        if 'distances' not in preds:
            # Safeguard — ak model nemá distance head, použij len detection loss
            loss_detach = torch.zeros(4, device=self.device)
            loss_detach[:3] = det_loss_detach
            return loss.sum() * batch_size, loss_detach

        # preds["distances"]: (bs, 1, num_anchors) → (bs, num_anchors)
        pred_distances = preds["distances"].squeeze(1)  # (bs, num_anchors)
        # Apply sigmoid — výstup je v [0, 1] (normalizovaná vzdialenosť)
        pred_distances = pred_distances.sigmoid()

        # Ground truth distances
        if 'distances' not in batch:
            raise KeyError(
                "Batch neobsahuje 'distances'. Upravil si dataset loader pre Dist-YOLO? "
                "Pozri Krok 5: modifikácia dataset.py"
            )

        # batch["distances"]: (total_gt_boxes,) alebo (total_gt_boxes, 1)
        gt_distances_flat = batch["distances"].to(self.device).view(-1)  # flat tensor

        # Remap gt_distances na per-batch shape pre indexovanie
        # Rovnaký princíp ako preprocess() pre bboxes
        targets = torch.cat(
            (batch["batch_idx"].view(-1, 1).to(self.device),
             gt_distances_flat.view(-1, 1)),
            dim=1
        )  # (N, 2): [batch_idx, distance]

        # Vytvor (batch_size, max_gt_per_img, 1) tensor
        nl = targets.shape[0]
        if nl == 0:
            # Žiadne GT targets
            gt_dist_per_batch = torch.zeros(batch_size, 0, 1, device=self.device)
        else:
            batch_idx = targets[:, 0].long()
            _, counts = batch_idx.unique(return_counts=True)
            counts = counts.to(dtype=torch.int32)
            gt_dist_per_batch = torch.zeros(batch_size, counts.max(), 1, device=self.device)
            offsets = torch.zeros(batch_size + 1, dtype=torch.long, device=self.device)
            offsets.scatter_add_(0, batch_idx + 1, torch.ones_like(batch_idx))
            offsets = offsets.cumsum(0)
            within_idx = torch.arange(nl, device=self.device) - offsets[batch_idx]
            gt_dist_per_batch[batch_idx, within_idx, 0] = targets[:, 1]

        # Teraz pre každý foreground anchor vyber target distance
        # target_gt_idx: (bs, num_anchors) — index do GT pre každý anchor
        # gt_dist_per_batch: (bs, max_gt, 1)

        # Gather: target_distances pre každý anchor
        # Expand indices na správny tvar
        gather_idx = target_gt_idx.unsqueeze(-1)  # (bs, num_anchors, 1)
        target_distances = torch.gather(
            gt_dist_per_batch,
            dim=1,
            index=gather_idx.clamp(min=0)
        ).squeeze(-1)  # (bs, num_anchors)

        # Iba pre foreground anchors
        if fg_mask.sum() > 0:
            dist_loss_per_anchor = self.dist_loss(
                pred_distances[fg_mask],
                target_distances[fg_mask]
            )
            loss[3] = dist_loss_per_anchor.mean()
        else:
            loss[3] = torch.tensor(0.0, device=self.device)

        # Aplikuj hyp weight
        loss[3] *= self.dist_gain

        # Detached loss pre logging
        loss_detach = torch.zeros(4, device=self.device)
        loss_detach[:3] = det_loss_detach
        loss_detach[3] = loss[3].detach()

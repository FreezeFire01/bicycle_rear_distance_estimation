...
def verify_image_label(args: tuple) -> list:
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    assert lb.shape[1] in {5, 6}, f"labels require 5 or 6 columns (Dist-YOLO), {lb.shape[1]} columns detected"
                    points = lb[:, 1:]
        ...
        if keypoint:
            ...
            # Ak je 6 stĺpcov, ponechaj všetky (5 štandard + 1 distance)
        elif lb.shape[1] == 5:
            lb = lb[:, :5]
            # 6 stĺpcový label sa nezmení (ponechá distance v 6. stĺpci)

...
"""
YOLODistDataset — dataset trieda pre Dist-YOLO
Rozširuje YOLODataset o čítanie a propagáciu distance ground truth.

Kľúčové zmeny oproti YOLODataset:
1. cache_labels: čítaj 6. stĺpec ako 'distances' do label dict
2. update_labels_info: pripoj distances k Instances
3. __getitem__: po augmentácii filtruj distances podľa zostávajúcich bboxov
4. collate_fn: agreguj distances do batchu

"""


# ============================================================
# PRIDAJ TENTO KÓD DO: ultralytics/data/dataset.py
# VLOŽ HO HNEĎ ZA class YOLODataset (pred YOLOMultiModalDataset)
# ============================================================

class YOLODistDataset(YOLODataset):
    """Dataset class for Dist-YOLO — extends YOLODataset with per-box distance ground truth.

    Reads 6-column labels: class_id cx cy w h distance_normalized
    The 'distances' field is propagated through training, kept aligned with bboxes.

    Note: Mosaic, mixup, cutmix and copy_paste augmentations are disabled by default
    because they would mix distance values from different images. Standard augmentations
    (flip, scale, color jitter) are preserved as they don't affect distance.
    """

    def __init__(self, *args, data=None, task: str = "detect", **kwargs):
        """Initialize Dist-YOLO dataset."""
        super().__init__(*args, data=data, task=task, **kwargs)

    def cache_labels(self, path=Path("./labels.cache")) -> dict:
        """Cache labels with distance values from 6. column."""
        x = {"labels": []}
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                ...
                nc += nc_f
                if im_file:
                    # === DIST-YOLO MODIFIKÁCIA ===
                    # Ak má label 6 stĺpcov, vytiahni distances
                    if lb.shape[1] >= 6:
                        cls = lb[:, 0:1]
                        bboxes = lb[:, 1:5]
                        distances = lb[:, 5:6]  # 6. stĺpec
                    else:
                        # Fallback: žiadne distance, použi -1
                        cls = lb[:, 0:1]
                        bboxes = lb[:, 1:5]
                        distances = np.full((lb.shape[0], 1), -1.0, dtype=np.float32)
    ...
    def build_transforms(self, hyp=None):
        """Build transforms — vypne mosaic/mixup/cutmix pre Dist-YOLO."""
        if self.augment:
            # Vypnúť augmentácie ktoré mixujú obrazy (a teda by zmiešali distances)
            hyp.mosaic = 0.0
            hyp.mixup = 0.0
            hyp.cutmix = 0.0
            hyp.copy_paste = 0.0
    ...
    def update_labels_info(self, label):
        """Update label format — drží distance synchronizované s bboxes."""
        # Vytiahni distances pred volaním parent
        distances = label.pop("distances", None)

        # Volaj pôvodnú metódu (ktorá pop-ne bboxes, segments, atď.)
        label = super().update_labels_info(label)

        # Distances naspäť do label dict
    ...
    def __getitem__(self, index: int) -> dict:
        """Get item with distances synchronized after augmentations."""
        label = self.transforms(self.get_image_and_label(index))

        # Po transformáciách: niektoré bboxes mohli byť odstránené (cropping atď.)
        # Treba synchronizovať distances s aktuálnymi bboxes
        if 'distances' in label and 'cls' in label:
            distances = label['distances']
            cls_count = len(label['cls']) if hasattr(label['cls'], '__len__') else label['cls'].shape[0]
            ...
            # Ak augmentácia odstránila niektoré bboxy, skráť aj distances
            # (Predpokladá že prvých N bboxov sú pôvodné — toto je aproximácia)
            if distances.shape[0] != cls_count:
                if distances.shape[0] > cls_count:
                    distances = distances[:cls_count]
                else:
                    # Doplň -1 ak chýbajú (nemalo by sa stať pri vypnutom mosaic)
                    pad = torch.full((cls_count - distances.shape[0], distances.shape[1] if distances.dim() > 1 else 1),
                                     -1.0)
                    distances = torch.cat([distances, pad], dim=0)
    ...
    def collate_fn(batch):
        """Collate batch — agreguj distances paralelne s bboxes."""
        # Vytiahni distances pred volaním parent collate
        distances_list = [b.pop('distances', None) for b in batch]

        # Volaj pôvodnú collate_fn
        new_batch = YOLODataset.collate_fn(batch)

        # Agreguj distances
        valid_distances = [d for d in distances_list if d is not None]
        if valid_distances:
            new_batch['distances'] = torch.cat(valid_distances, 0)

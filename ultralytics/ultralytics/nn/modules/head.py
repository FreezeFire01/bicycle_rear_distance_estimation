...
__all__ = "OBB", "Classify", "Detect", "DetectDist", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class DetectDist(Detect):
    """YOLO Detect head s distance prediction branch.

    Rozširuje štandardný Detect o siedmy výstup — normalizovanú vzdialenosť [0,1].
    Používa rovnaký multi-scale mechanizmus ako Detect, s novou cv4 branch.

    Attributes:
        cv4 (nn.ModuleList): Convolution layers for distance regression.
    """

    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize DetectDist head.
        ...
        # Rovnaká štruktúra ako cv3 (classification) ale s 1 výstupom
        c4 = max(ch[0] // 4, 16)  # channels — malej, keďže 1 výstup

        self.cv4 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c4, 3),
                Conv(c4, c4, 3),
                nn.Conv2d(c4, 1, 1),  # 1 kanál — distance
            ) for x in ch
        )

        # no = bbox DFL + classes + distance
        self.no = nc + self.reg_max * 4 + 1

    def forward_head(
            self, x, box_head=None, cls_head=None, dist_head=None
    ):
        """Rozšírenie forward_head o distance prediction.

        Args:
            x (list[Tensor]): Multi-scale feature maps from backbone.
            box_head, cls_head, dist_head (nn.ModuleList): Prediction branches.

        Returns:
            dict: Predictions with 'boxes', 'scores', 'distances', 'feats'.
        """
        ...
        # Distance prediction (iba ak dist_head je poskytnutý)
        if dist_head is not None:
            distances = torch.cat(
                [dist_head[i](x[i]).view(bs, 1, -1) for i in range(self.nl)],
                dim=-1
            )
    ...
    @property
    def one2many(self):
        """Returns one-to-many head components including distance."""
        return dict(box_head=self.cv2, cls_head=self.cv3, dist_head=self.cv4)

    @property
    def one2one(self):
        """Returns one-to-one head components (no distance for one2one)."""
        # One-to-one režim nemá distance head, používa sa pôvodná cv2/cv3
        return dict(box_head=self.one2one_cv2, cls_head=self.one2one_cv3)

    def _inference(self, x):
        """Decode predictions including distance.

        Args:
            x (dict): Dictionary with 'boxes', 'scores', optional 'distances'.

        Returns:
            Tensor: Concatenated [decoded_boxes, class_probs, distances]
        """
        # Pôvodné box + class decoding
        dbox = self._get_decode_boxes(x)

        # Output tensor
        if 'distances' in x:
            # Apply sigmoid to get [0, 1] range for distance
            dist = x['distances'].sigmoid()
            return torch.cat((dbox, x["scores"].sigmoid(), dist), 1)
        else:
            return torch.cat((dbox, x["scores"].sigmoid()), 1)

    def bias_init(self):
        """Initialize biases for all heads including distance."""
        super().bias_init()

        # Init distance bias — začneme v strede rozsahu (0.5 sigmoided)
        # sigmoid^-1(0.5) = 0, takže bias = 0 dáva strednú vzdialenosť ~45m pri MAX=90m
        for a in self.cv4:
            a[-1].bias.data[:] = 0.0
